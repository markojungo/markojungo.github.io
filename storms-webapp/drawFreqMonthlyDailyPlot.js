drawFreqMonthlyDailyPlot = function() {
    ...
    var setupScales = function(data) {
        xScale.domain(d3.extent(data, d => d.monthday));
    ...
    var areaGen = d3.area()
                    .x((d) => {return xScale(d.data.monthday) })
                    .y0((d) => yScale(d[0]))
                    .y1((d) => yScale(d[1]));

    var xAxis = d3.axisBottom(xScale).tickFormat(d3.timeFormat('%b-%d'))
            ...
            svg.append('rect')
                ...
                .on("mousemove", function(e, d) {
                    ...
                    var closest_i = d3.scan(data, (a, b) => {
                        ...
                        var aMonthday = parseInt(parser(a.monthday)),
                            bMonthday = parseInt(parser(b.monthday)),
                            xMonthday = parseInt(parser(x0));
                        return Math.abs(aMonthday - xMonthday) - Math.abs(bMonthday - xMonthday);
                    });
                    var closest = data[closest_i];
                    var closestX = closest.monthday;
                    var rels = [];
                    ...
                    var {monthday, ...rest} = closestRow.data;
                    ...
                    d3.select('body#tip2').html(() => {
                        return  `<div xmlns='http://www.w3.org/1999/xhtml' class='container p-1' style='color: ${minorMajor == 'major' ? 'white' : 'black'}'>
                                    ...
                                    <p style='font-size: 11px; margin: 0'><b>Month:</b> ${d3.timeFormat('%B, %d')(closestX)}</p>
